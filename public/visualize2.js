...
function pathFind(initial, target) {
    ...
    let start = graph.grid[initial.x][initial.y];
...
var myGameArea = {
    ...
    start: function () {
        ...
        for (let i = 0; i < robots.length; i++) {
            //console.log([robots[i].x, robots[i].y])
            let temp = pathFind({ x: robots[i].x, y: robots[i].y }, { x: exit.x, y: exit.y })
            for (let j = 0; j < temp.length; j++) {
                tempPath.push(new component(1, 1, "cyan", temp[j].x, temp[j].y))
            }
        }
